...
function processImport(data,target){var count=0;if(target==='sheet'){data.forEach(function(r){var n=r.Project||r.project||r.Name||r.name||'';if(!n)return;state.sheet.push({id:uid('sh'),name:n,type:r.Type||r.type||'web',status:r.Status||r.status||'testing',received:r.Received||r.received||'',mailed:r.Mailed||r.mailed||'',started:r.Started||r.started||'',submitted:r.Submitted||r.submitted||'',cycles:parseInt(r.Cycles||r.cycles)||0,bugs:parseInt(r.Bugs||r.bugs)||0,remarks:r.Remarks||r.remarks||''});count++;});save();renderSheet();closeM('mSheetImp');}else if(target==='worksheet'){data.forEach(function(r){var t=r['Work Title']||r.Title||r.title||'';if(!t)return;var d=r.Date||r.date||ds();state.worksheet.push({id:uid('ws'),date:d,day:r.Day||r.day||dayName(d),title:t,desc:r['Work Description']||r.Description||r.desc||''});count++;});save();renderWs();closeM('mWsImp');}else{data.forEach(function(r){var n=r.Test||r['Test Name']||r.test||r.Name||r.name||'';if(!n)return;var cn=r.Category||r.category||'Imported';var m=r.Module||r.module||'web';if(m!=='web'&&m!=='app')m='web';var cat=state.categories.find(function(c){return c.name.toLowerCase()===cn.toLowerCase()&&c.module===m;});if(!cat){var cid=uid('cat');cat={id:cid,icon:'📥',name:cn,module:m,tests:[]};state.categories.push(cat);ui.collapsed[cid]=true;}cat.tests.push(T(uid('imp'),n,r.Priority||r.priority||'medium',m));count++;});save();popCatFlt();render();closeM('mImp');}toast('success','Imported',count+' items');}
